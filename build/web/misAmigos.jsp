<%-- 
    Document   : misAmigos
    Created on : 4/12/2015, 09:23:04 AM
    Author     : Gerardo
--%>
                ...
                <div class="row">
                    <%
                        ...
                        if(conexion.conecta() != 0)
                        {
                            ...
                            sql = "SELECT u.id_usuario,concat(u.nombre, ' ', u.apellidos) as usuario FROM usuarios u WHERE u.id_usuario IN(SELECT la.id_amigo FROM usuarios u, lista_amigos la WHERE u.id_usuario=la.id_amigo AND la.id_usuario="+id_usuario+") and u.id_usuario<>"+id_usuario;
                        }
                        ...
                        if (result.next()) 
                        {
                            ...
                            {
                                %>
                                <p><%=result.getString("usuario")%>&nbsp;&nbsp;&nbsp;<a href="eliminarAmigo.jsp?id_amigo=<%=result.getString("u.id_usuario")%>">Eliminar</a></p>
                               <%
                            }
                        }
                        else
                            out.print("<p>No has agregado a ningún amigo</p>");
                        result.close();
                      conexion.cerrar();%>
                </div>
            </div>
